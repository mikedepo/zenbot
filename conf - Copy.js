...
// mongo configuration
c.mongo = {}
c.mongo.db = process.env.ZENBOT_MONGODB_DATABASE || 'zenbot2'

// Must provide EITHER c.mongo.connectionString OR c.mongo.host,port,username,password
c.mongo.connectionString = process.env.ZENBOT_MONGODB_CONNECTION_STRING || 'mongodb://localhost:27017/?readPreference=primary&ssl=false'

// The following is not needed when c.mongo.connectionString is provided:
c.mongo.host = process.env.ZENBOT_MONGODB_HOST || null
c.mongo.port = process.env.ZENBOT_MONGODB_PORT || null
...
c.selector = process.env.ZENBOT_DEFAULT_SELECTOR || 'binance.STMX-USDT'
// name of default trade strategy
c.strategy = process.env.ZENBOT_DEFAULT_STRATEGY || 'bollinger'
...
// Stop loss % (0 to disable)
c.sell_stop_pct = process.env.ZENBOT_SELL_STOP_PCT || 0
// Trailing buy -> buy if price surges above this % of sold price (0 to disable)
c.buy_stop_pct = process.env.ZENBOT_BUY_STOP_PCT || 0
// Trailing stop-loss enable -> enable trailing sell stop when reaching this % profit (0 to disable)
c.profit_stop_enable_pct = process.env.ZENBOT_PROFIT_STOP_ENABLE_PCT || 0
// Trailing stop-loss (sell at %)-> maintain a trailing stop this % below the high-water mark of profit
...
// Buy percentage -> buy with this % of currency balance (WARNING : sim won't work properly if you set this value to 100)
c.buy_pct = process.env.ZENBOT_BUY_PCT || 99
// Buy amount -> buy with this $ of currency balance (WARNING : sim won't work properly if you set this value to 100)
c.deposit = process.env.ZENBOT_DEPOSIT || 20
// Sell percentage -> sell with this % of asset balance (WARNING : sim won't work properly if you set this value to 100)
c.sell_pct = process.env.ZENBOT_SELL_PCT || 100
// Timeout for buy -> ms to adjust non-filled order after
c.order_adjust_time = process.env.ZENBOT_ORDER_ADJUST_TIME || 5000
// Sell only with profits -> avoid selling at a loss below this pct set to 0 to ensure selling at a higher price...
...
// Pull order status every (ms) -> ms to poll order status
c.order_poll_time = process.env.ZENBOT_ORDER_POLL_TIME || 5000
// Order cancel timeout -> ms to wait for settlement (after an order cancel)
c.wait_for_settlement = process.env.ZENBOT_WAIT_FOR_SETTLEMENT || 5000
// Buy below mark -> % to mark down buy price for orders
c.markdown_buy_pct = process.env.ZENBOT_MARKDOWN_BUY_PCT || 0
// Sell over mark -> % to mark up sell price for orders
c.markup_sell_pct = process.env.ZENBOT_MARKUP_SELL_PCT || 0
// maker or taker -> become a market taker (high fees) or a market maker (low fees)
...
// pay using BNB -> use separated fee currency such as binance's BNB.
...
// Simulation investment money -> amount of currency to start simulations with
c.currency_capital = process.env.ZENBOT_CURRENCY_CAPITAL || 1000
// Simulation investment crypto -> amount of asset to start simulations with
...
c.notifiers.telegram.bot_token = process.env.ZENBOT_TELEGRAM_BOT_TOKEN || "1736396074:AAEilpFVLtR6ogm1WAFC4g66Ro2YUM1iVZU"
c.notifiers.telegram.chat_id = process.env.ZENBOT_TELEGRAM_CHAT_ID || "-520739488" // the id of the chat the messages should be send in
